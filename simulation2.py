...
def Main():
    ...
    if file.tell() == 0:
        writer.writerow(['tcp', 'ber', 'mean', 'off99', 'off95'])
...
def Simulate(ber):
    ...
    # variable delay
    delayVar = ns.CreateObject[ns.UniformRandomVariable]()
    delayVar.SetAttribute('Min', MIN_DELAY)
    delayVar.SetAttribute('Max', MAX_DELAY)
    # wifi with no signal loss and variable delay
    wifiChannel = ns.YansWifiChannelHelper()
    wifiChannel.SetPropagationDelay('ns3::RandomPropagationDelayModel',
                                    'Variable', ns.PointerValue(delayVar))
    ...
    csma.SetChannelAttribute('Delay', ns.TimeValue(ns.Time("1ms")))
    # ethernet nics
